...
class lib_verifycode
{
	...
	public function __construct($code_key = '', $width= '', $height = '', $code_num = 4)
	{
	    if(empty($code_key))
	    {
	        $code_key = $this->get_cur_user_key();
	    }
	    $this->_code_key = md5('my_' . $code_key);

		$this->CI = & get_instance();
	    
	    $this->_width = !empty($width) ? $width : $this->_width;
	    $this->_height = !empty($height) ? $height : $this->_height;
	    $this->_code_num = !empty($_code_num) ? $_code_num : $this->_code_num;
	}

	public function get_code($type=2)
	{
		$code_type = $type == 4 ? mt_rand(1, 3) : $type;
	...
	public function get_math_code()
	{
    	...
    	$this->CI->lib_redis->setTimeout($this->_code_key, 1800);
    ...
    public function check_code($code)
    {
		...
		if($verify_code && $verify_code == $code) {
    	    ...
    	    //删除验证码
    	    $this->CI->lib_redis->delete($this->_code_key);
    	}
